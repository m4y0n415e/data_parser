...
def analise(df):
    ...
    # implement calculation of t distribution
...
def loc_stats(df):
        ...
        mean_pack = round(df_working['patientcard_packyears_packyearsvalue'].mean(), 1)
        print("SD of pack-years: ", statistics.stdev(df_working['patientcard_packyears_packyearsvalue'].dropna(), xbar=mean_pack))
        mean_pack_women = round(df_working[df_working['patient_sex_shortdesc'] == 'K']['patientcard_packyears_packyearsvalue'].mean(), 1)
        mean_pack_men = round(df_working[df_working['patient_sex_shortdesc'] == 'M']['patientcard_packyears_packyearsvalue'].mean(), 1)
        median_pack = df_working['patientcard_packyears_packyearsvalue'].median()
